...
`watch-model-forecast-probe` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for forecast workflows, centered on security rule linting, safe and unsafe fixtures, and remediation hints.
...
The score is deterministic. The current threshold is 172. Risk has a penalty of 7, latency has a penalty of 4, and weight contributes a bonus of 6.
